...
const ProductDetail = () => {
  ...
  const { products, addToCart, removeFromCart, cartItems, ThemVaoGioHang } = useContext(StoreContext)
  ...
  const [others, setOthers] = useState(products.filter((e) => e._id !== product._id).slice(-6));
  const date = new Date();
    ...
    <div className='flex flex-col md:flex-row p-5 gap-5 items-start'>
        <div className='w-full md:w-1/3 flex flex-col justify-center items-center px-5 py-4 gap-5 bg-white drop-shadow-[0px_4px_4px_rgba(0,0,0,0.25)] rounded-lg'>
          <div className='md:drop-shadow-[0px_4px_4px_rgba(0,0,0,0.25)]'>
            <img className='w-full md:w-[200px] rounded-[10px]' src={product.imageUrl ? product.imageUrl : product.image} alt="" />
          </div>

          <div className='md:flex gap-2 hidden'>
            <img className='hidden w-[70px] md:flex  drop-shadow-[0px_4px_4px_rgba(0,0,0,0.25)]' src={product.imageUrl ? product.imageUrl : product.image} alt="" />
            <img className='hidden w-[70px] md:flex  drop-shadow-[0px_4px_4px_rgba(0,0,0,0.25)]' src={product.imageUrl ? product.imageUrl : product.image} alt="" />
            <img className='hidden w-[70px] lg:flex drop-shadow-[0px_4px_4px_rgba(0,0,0,0.25)]' src={product.imageUrl ? product.imageUrl : product.image} alt="" />
            <img className='hidden w-[70px] 2xl:flex drop-shadow-[0px_4px_4px_rgba(0,0,0,0.25)]' src={product.imageUrl ? product.imageUrl : product.image} alt="" />
          </div>
          <div className='w-full'>
            <span className='hidden md:flex font-bold text-[20px] leading-[28px] text-[#1C1C28]'>Đặc điểm nổi bật </span>
          </div>
           <ul className=''> {/*list-disc list-inside */}
            <li className='hidden md:block pl-2 font-normal text-[16px] leading-[19px] text-black'>{product.description}</li>
          </ul>
        ...
        <div className='flex flex-col w-full gap-5'>
          <div className="flex flex-col md:flex-row w-full gap-5">
              ...
              <div className="w-[200px] h-[40px] mt-2 bg-blue-500 text-white rounded-md flex items-center justify-center cursor-pointer" onClick={() => ThemVaoGioHang(product._id)
          ...
          <div className="gap-4 flex flex-col justify-center items-center px-5 py-3 isolation-isolate  bg-[#E8F3FB] shadow-[0px_0px_10px_rgba(0,0,0,0.25)] rounded-[10px]">
            ...
            <div className='w-full flex flex-col'>
              <div className='flex border-t-[1px] border-black p-3'>
                <span className="hidden lg:block w-1/5 font-inter font-normal md:text-[24px] md:leading-[29px] text-center text-black">Display</span>
                <span className="lg:w-4/5 w-full justify-center items-center flex font-inter font-normal text-[18px]  md:text-[24px] leading-[24px] md:leading-[29px] text-center text-black">Foldable LTPO OLED, 120Hz, HDR10+, 1600 nits</span>
              </div>
              <div className='flex border-t-[1px] border-black p-3'>
                <span className="hidden lg:block w-1/5 font-inter font-normal md:text-[24px] md:leading-[29px] text-center text-black">Software</span>
                <span className="lg:w-4/5 w-full justify-center items-center flex font-inter font-normal text-[18px]  md:text-[24px] leading-[24px] md:leading-[29px] text-center text-black">Android 14, upgradable to Android 15</span>
              </div>
              <div className='flex border-t-[1px] border-black p-3'>
                <span className="hidden lg:block w-1/5 font-inter font-normal md:text-[24px] md:leading-[29px] text-center text-black">Main Camera</span>
                <span className="lg:w-4/5 w-full justify-center items-center flex font-inter font-normal text-[18px]  md:text-[24px] leading-[24px] md:leading-[29px] text-center text-black">48 MP, f/1.7, 25mm | 10.8 MP, f/3.1, 112mm | 10.5 MP, f/2.2, 127</span>
              </div>
              <div className='flex border-t-[1px] border-black p-3'>
                <span className="hidden lg:block w-1/5 font-inter font-normal md:text-[24px] md:leading-[29px] text-center text-black">Battery</span>
                <span className="lg:w-4/5 w-full justify-center items-center flex font-inter font-normal text-[18px]  md:text-[24px] leading-[24px] md:leading-[29px] text-center text-black">4650 mAh, non-removable | 21W wired | 7.5W wireless</span>
              </div>
            </div>
          </div>
          <div className="gap-4 flex flex-col justify-center items-center px-5 py-3 isolation-isolate  bg-[#E8F3FB] shadow-[0px_0px_10px_rgba(0,0,0,0.25)] rounded-[10px]">
